...
public class Jeu implements Serializable {
	...
	void jouer(Point p1, Point d) {
		 ...
		 * 
		 * } } } else deplacer(p1,d,1);//p1 et d doivent ÃƒÂªtre valide
		 */
	...
	// vÃƒÂ©rifie si le pion la position (l,c) est un pion mangeur;
	boolean pionMangeur(int pion, int l, int c) {
		// vÃƒÂ©rifie au niveau de la verticale et l'horizentale
	...
	SequenceListe<SequenceListe<Point>> pionMangeurApproche(int pionmangeur, int l, int c) {
		 ...
		 * 
		 * list=new SequenceListe<>(); list.insereTete(new Point(1,0));
		 * list.insereTete(new Point(l-2,c)); tuple.insereTete(list); //vÃƒÂ©rifie au
		 * niveau des deux diagonales if(l%2==c%2)//vÃƒÂ©rifie que le mouvement en
	...
	SequenceListe<SequenceListe<Point>> pionMangeurEloignement(int pionmangeur, int l, int c) {
		 ...
		 * vÃƒÂ©rifie si le pion choisis n'est pas dans l'extremitÃƒÂ© du plateau
		 * //vÃƒÂ©rifie au niveau de la verticale et l'horizentale if(
